    ...
    .upsert(
      {
        ...
        exercise_sort_order: exerciseSortOrder ?? null,
        is_deleted: true,
...
async function syncDeletedWorkoutSession(
  ...
  const exerciseSortOrder = getExerciseSortOrderMap(session.user_id, session.local_id);
  ...
  for (const set of setsToFinalize) {
    const didDelete = await syncDeletedWorkoutSet(
      ...
      syncedSessionId,
      deletedAt,
      exerciseSortOrder.get(set.exercise_id) ?? null
    );
    ...
    try {
      ...
      if (activeSets.length > 0) {
        const setRows = activeSets.map((set) => ({
          ...
          weight: set.weight,
          completed: Boolean(set.completed),
          exercise_sort_order: exerciseSortOrder.get(set.exercise_id) ?? null,
          is_deleted: false,
          deleted_at: null,
        }));
